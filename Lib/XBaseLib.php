...
class XBaseLib {
	// class-members
	protected $num; // the (n_base) value
	protected $base; // the base
	protected $dec_num; // the (decimal) value
	protected $alnum_digit; // the used set of digits
	protected $max_base; // the max possible base to convert
	...
	protected function calc_dec($number) {
	...
	protected function build_nBase_number() {
	...
	protected function get_highest_numPart($dec_num, $base) {
		if ($dec_num == 0)
			return "0";
		$dec = 0;
		$pos = 0;
	...
	protected function changeAlnumDigits($uc) {
		switch ($uc) {
			case "upper":
