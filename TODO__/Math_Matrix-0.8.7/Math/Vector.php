...
class Math_Vector {
     ...
     * @see setData()
	 */
	public function Math_Vector($data=null) /*{{{*/
	{
        if (!is_null($data)) {
            $this->setData($data);
        }
	 ...
	 */
	public function distance($vector, $type='cartesian')/*{{{*/
	{
		switch ($type) {
			case 'manhattan' :
			case 'city' :
	 ...
	 * @return	float on success, a PEAR_Error object on failure
	 */
	public function manhattanDistance($vector) /*{{{*/
	{
		$n = $this->size();
		$sum = 0;
		if (Math_VectorOp::isVector($vector))
	 ...
	 * @return	float on success, a PEAR_Error object on failure
	 */
	public function chessboardDistance($vector) /*{{{*/
	{
		$n = $this->size();
		$sum = 0;
		if (Math_VectorOp::isVector($vector))
